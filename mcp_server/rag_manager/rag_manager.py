...
class RAGManager:
    ...
    def _get_adaptive_depth(self, latency_info: Dict) -> int:
        """Determines the appropriate recursion depth based on the remaining time budget."""
        if not latency_info or not latency_info.get("budget_ms"):
            return 1 # Default depth if no budget is specified
        ...
        if remaining_budget_ms > 4000:
            return 2 # Full depth search
        elif remaining_budget_ms > 1500:
            return 1 # Limited search
        else:
            return 0 # Seed URLs only, no recursion

    async def perform_research(self, topic: str, latency_info: Dict, num_sources: int = 2) -> str:
        """
        Performs research on a given topic with adaptive depth.
        """
